...
export const getTripSlugFromUrl = (url: string) => url.match(new RegExp(`^.*${TRIP_PAGE.replace(':slugOrId', '([^/?]+)')}`))?.[1]
export const getHostSlugFromUrl = (url: string) => url.match(new RegExp(`^.*${HOST_PAGE.replace(':slugOrId', '([^/?]+)')}`))?.[1]
